...
def get_exchange_rate_data(currency_code, last_page_num):
    ...
    for page_num in range(1, last_page_num+1):
        ...
        dfs = pd.read_html(url, header=1,encoding='cp949') #표일 때 pandas 로 한꺼번에 가져올 수 있음 
...
def exchange_main():
    ...
    if clicked:
        ...
        #차트 그리기(선 그래프, 날짜형식으로 바꾸고 pandas이용)
        ...
        csv_data = df_exchange_rate.to_csv()#괄호안에 적지 않으면 가상 메모리 안에 적재
        #엑셀 데이터 변환
        excel_data = BytesIO() #메모리 버퍼에 바이너리 객체 생성
        df_exchange_rate.to_excel(excel_data)


        #2개의 세로단을 구성해서 버튼 만들어
        col = st.columns(2) #리스트형식으로 됨
        # col1,col2 = st.columns(2)도 가능
        with col[0]:
